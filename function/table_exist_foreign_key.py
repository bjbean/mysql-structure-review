...
"""************************************************************
名称
    f_rule_table_exist_foreigh_key
描述
    获得数据库指定Schema是否存在外键并计算扣分
参数
    p_parms        list            参数列表
...
返回值
    ...
    'scores'    float   扣分
    'records'   list    外键信息(表名,大小)
************************************************************"""
def f_rule_table_exist_foreign_key(p_parms):
    ...
    cursor.execute("""
        ...
        where CONSTRAINT_SCHEMA='"""+l_username+"""' and
            table_schema='"""+l_username+"""' and
