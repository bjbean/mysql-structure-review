...
参数
    ...
    p_parms[2]     l_column_num    int     字段数量
    p_parms[3]     l_threshold     int     扣分阀值
    p_parms[4]     l_max_value     int     扣分上限
返回值
    dict{'scores':xxx,'records':xxx}
    'scores'    float   扣分
    'records'   list    表信息(表名,字段数)
************************************************************"""
def f_rule_table_column_num(p_parms):
    [l_dbinfo, l_username, l_column_num, l_weight, l_max_value] = [p_parms[0], p_parms[1], int(p_parms[2]), int(p_parms[3]), int(p_parms[4])]
    ...
    cursor.execute("""
        ...
        GROUP BY table_name
        HAVING count(1)>"""+str(l_column_num)
        )
